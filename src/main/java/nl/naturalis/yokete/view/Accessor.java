...
public interface Accessor<T> {
   ...
   * true {@code null} values and the variable not being present in the source data at all. True
   * {@code null} values are valid valid values that will be stringified somehow by a {@link
   * Stringifier} (e.g. to an empty string or to "&amp;nbsp;"). If the variable could not be mapped
   * to a bean property at all, this method must return the special value {@link #ABSENT}. If a
   * {@link RenderSession} receives this value for a particular variable, it will assume you don't
   * want to that particular variable just yet. (In the end, of course, all template variables must
   * be set before the template can be rendered.)
   *
   * @param from The object (supposedly) containing the value
   * @param varName The name of the template variable
   * @return The value of the variable
   */
  Object getValue(T from, String varName) throws RenderException;
}
